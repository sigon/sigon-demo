...
<body class="iframe">
<form action="bankList.action" id="listForm" method="POST">
    ...
    <table>
        ...
        [#list page.content as bank]
            <tr data-val="${bank.bank.id}">
                ...
                <td>[#if bank.pendingBalance??][@number num = bank.pendingBalance][/@number][#else]0.00[/#if]</td>
                <td>[#if bank.availableBalance??][@number num = bank.availableBalance][/@number][#else]0.00[/#if]</td>
            </tr>
        [/#list]
