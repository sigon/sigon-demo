...
<body>
<form id="listForm" action="/totalAmount/line.action" method="post">
    <h2>Transaction Summary By gateway</h2>
    <menu>
        <div class="form">
            <p class="f_2p">
                <label class="f_info">
...
<script>
    seajs.use(["alert","echarts"],function(alertM){
        [#--var dateType = $("#dateType").on("change",function(){--]
            [#--var html='<input id="wdateStart" name="timeStart" type="text" style="width:90px;" onClick="WdatePicker({dateFmt:\''+dateType.val()+'\'})" class="Wdate regionV" value="${dateStart}"><input id="wdateEnd" name="timeEnd" type="text" style="width:90px;" onClick="WdatePicker({dateFmt:\''+dateType.val()+'\'})" class="Wdate regionV" value="${dateEnd}">';--]
            [#--$("#dateSpan").html(html);--]
        [#--})--]
